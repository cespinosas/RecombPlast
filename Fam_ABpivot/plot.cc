...
void uddd(string dirarr, double *dis1, double *dis2, int nudats) {
  ...
  flusal.open(orden.c_str());
  flusal << "*Distance between ABxA parents:\n";
  ...
  flusal << "*Distance between ABxAB parents:\n";
...
void uddutu(int qfen0B_1A, string mmmms, string dirarr, double **off1, double **off2, double **wife1, double **wife2, int nudats, int nodes) { //deoff pasar rebcorrecta
  ...
  flusal << "library(ggplot2)\n";
  flusal << "a <- read.csv(file = \""<< eldir << "data" << ".txt\", head=FALSE, sep =\"\\t\")\n";
  skahis_idl(flusal, eldir+elfen+"off2n1_off1n1", 4, 1, 1, 1, "AB\'%*%\'A offspring\\'s access to \'*italic("+elfen+")*\' through plasticity", "AB\'%*%\'AB offspring\\'s access to \'*italic("+elfen+")*\' through plasticity", 0.15);

  skahis_idl(flusal, eldir+elfen+"off2n2_off1n2", 5, ((nodes*(nodes-1))/2.0), 2, ((nodes*(nodes-1))/2.0), "AB\'%*%\'A offspring\\'s access to \'*italic("+elfen+")*\' through plasticity", "AB\'%*%\'AB offspring\\'s access to \'*italic("+elfen+")*\' through plasticity", 0.15);
  skahis_idl(flusal, eldir+elfen+"off2mu_off1mu", 6, 1, 3, 1, "AB\'%*%\'A offspring\\'s access to \'*italic("+elfen+")*\' through mutation", "AB\'%*%\'AB offspring\\'s access to \'*italic("+elfen+")*\' through mutation", 0.15);
  wilcoxon(flusal, eldir+"stats", 1, 4, 'g', elfen+"_off_f1_n1", elfen+"_off_f2_n1");

  wilcoxon(flusal, eldir+"stats", 2, 5, 'g', elfen+"_off_f1_n2", elfen+"_off_f2_n2");
  wilcoxon(flusal, eldir+"stats", 3, 6, 'g', elfen+"_off_f1_mu", elfen+"_off_f2_mu");
...
void uddutd(int qfen0B_1A, string dirarr, double **wife1, double **wife2, int **nat1, int **nat2, int nudats, int nodes) { //dirarr con /terminal
  ...
  flusal << "library(ggplot2)\n";
  flusal << "a <- read.csv(file = \""<< eldir << "data" << ".txt\", head=FALSE, sep =\"\\t\")\n";
  skahis_idl(flusal, eldir+elfen+"_natf2_natf1", 11, 1, 10, 1, "AB\'%*%\'A offspring with native \'*italic("+elfen+")*\'", "AB\'%*%\'AB offspring with native \'*italic("+elfen+")*\'", 0.15);
  wilcoxon(flusal, eldir+"stats", 10, 11, 'g', elfen+"_off_f1_native", elfen+"_off_f2_native");
  flusal.close();
  orden = "Rscript "+eldir+"paR.sh";
  system(orden.c_str());
  return;
