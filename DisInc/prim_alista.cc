...
int main(int argc, char **argv) {
  ...
  int nunods, nuedges, dcia, dcib, dab;
    nunods = 12;
    nuedges = 36;
    dcia = 3;
    dcib = 3;
    ...
    bas.open_ofstream(fs, losdir + "run_ABp_" + bas.inttostring(nunods) + "_" + bas.inttostring(nuedges) + "_" + bas.inttostring(dcia) + "_" + bas.inttostring(dcib) + "_" + bas.inttostring(dab)+".sh");
    fs << "#!/bin/bash\n";
    fs << "#PBS -l walltime=498:00:00\n";
    fs << "#PBS -l nice=19\n";
    fs << "#PBS -l nodes=1:ppn=1\n";
    fs << "#PBS -q batch\n";
    fs << "cd $PBS_O_WORKDIR\n";
    fs << "../../../../increg ${PBS_ARRAYID} " << nunods << " " << nuedges << " " << dcia << " " << dcib << " " << dab << " " << "20\n";
    fs.close();
