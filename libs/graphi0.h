...
public:
  ...
  GraphI0(int n, Alea& jacta, bool dirnw);
  void make_nw(int n, bool dirnw);
  void copy(GraphI0 &templ);
  GraphI0(int n, double c, Alea& jacta, bool dirnw);
  void make_nw(int n, double c, bool dirnw);
  GraphI0(int n, int e, Alea& jacta, bool dirnw);
  void make_nw(int n, int e, bool dirnw);
  int rnd_nw_with_2fp1tn1(int n, int e, int *fp1, int *fp2, int *ic1); ///nuevisimo 2 fixed points, one with specific trajectory//equivalent to make_robust_nw **
  ...
  //analyses
  int distance_from_nw(GraphI0& otra);
  bool equal_adjmat(GraphI0& otra);
  bool equal_nw(GraphI0 &templ);
  ...
  //for evolution
  void build_offspring(GraphI0 &mother, GraphI0 &father, bool *frommom);
  void mate(GraphI0 &mother, GraphI0 &father);
...
private:
  ...
  int *degree;
  bool yadeg;
	int *degdist; // falta poner en GraphI0 y graphc
	int *odegdist;
	int *idegdist;
	bool yadegdist; // falta poner en GraphI0 y graphc... recordar que falta tambien en inicializadas
  int *edo;
  int *ima;
  int **attractor;
  int atsize;
